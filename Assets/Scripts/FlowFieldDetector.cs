...
public class FlowFieldDetector : MonoBehaviour
{
    ...
    public List<FlowFieldManager> FlowFields = new List<FlowFieldManager>();

    private void Start()
    {
        // detect items in scene with "FlowField" tag
        GameObject[] flowFields = GameObject.FindGameObjectsWithTag("FlowField");

        // add those items to list
        foreach (GameObject ff in flowFields)
        {
            FlowFields.Add(ff.GetComponent<FlowFieldManager>());
        }
    }
    ...
    public Vector3 FlowBoundry()
    {
        for (int i = 0; i < FlowFields.Count; i++)
        {
            ...
            if (transform.position.x > FlowFields[i].gridStartX &&
                transform.position.x < FlowFields[i].width + FlowFields[i].gridStartX && 
                transform.position.y > FlowFields[i].gridStartY && 
                transform.position.y < FlowFields[i].height + FlowFields[i].gridStartY)
            {
                force = FlowFields[i].GetForceForPosition(transform.position);
